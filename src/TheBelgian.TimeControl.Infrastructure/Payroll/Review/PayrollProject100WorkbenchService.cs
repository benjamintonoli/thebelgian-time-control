...
internal sealed class PayrollProject100WorkbenchService(
    ...
    PayrollProject100GpsCache gpsCache,
    PayrollProject100GpsContextCache gpsContextCache,
    PayrollProject100DaySourceCache daySourceCache,
    PayrollProject100QueueCache queueCache,
    PayrollProject100BonMemoCache bonMemoCache,
    PayrollProject100MonthContextCache monthContextCache,
    PayrollProject100HfdCache hfdCache,
    ...
    ILogger<PayrollProject100WorkbenchService> logger) : IPayrollProject100WorkbenchService
{
    ...
    public async Task<PayrollProject100WorkbenchPage> GetShellAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
    ...
    {
        EnsureEnabled();
        var scopedFilter = filter with { Category = PayrollReviewCategory.Project100 };
        ...
        return new PayrollProject100WorkbenchPage(
            year,
            month,
            queue.AdminSummary,
            cases,
            ...
            Detail: null,
            new PayrollProject100WorkbenchMetrics(0, 0, 0, GpsDeferred: true, MonthContextHit: ready),
            previews,
            MonthContextPending: !ready,
            MonthContextReady: ready);
    }

    public async Task<PayrollProject100WorkbenchPage> GetCoreDetailAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
        CancellationToken cancellationToken)
    {
        ...
        if (selected is null)
        {
            return new PayrollProject100WorkbenchPage(
                ...
                null,
                null,
                new PayrollProject100WorkbenchMetrics(
    ...
    public async Task<PayrollProject100GpsLoadResult> GetGpsContextAsync(
        int year,
        int month,
        string adminCaseKey,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (gpsContextCache.TryGet(adminCaseKey, out var cachedContext) && cachedContext is not null)
        {
            ...
            if (prefetchNext)
            {
                var queue = await GetAdminQueueCachedAsync(
                    year,
                    month,
                    new PayrollReviewQueueFilter(PayrollReviewCategory.Project100, Scope: PayrollReviewQueueScope.All),
                    cancellationToken);
            ...
            return new PayrollProject100GpsLoadResult(
                adminCaseKey,
                string.Empty,
                default,
                cachedContext,
        ...
        if (selected is null)
        {
            return new PayrollProject100GpsLoadResult(
                ...
                new PayrollProject100GpsContext(
                    false,
                    PayrollProject100WorkbenchBuilder.MissingGpsSummary,
                    [],
                    "Missing",
                    null,
                    []),
        ...
        var gpsContext = PayrollProject100WorkbenchBuilder.BuildDetail(
            selected,
            performances,
            [],
            evidence,
    ...
    public PayrollProject100GpsCacheHint GetGpsCacheHint(string resourceId, DateOnly workDate)
    {
        ...
        {
            return PayrollProject100GpsCacheHint.Unknown;
        }

        if (evidence is null || !evidence.HasUsableTrips)
        {
            return PayrollProject100GpsCacheHint.Unavailable;
        }

        return PayrollProject100GpsCacheHint.Available;
    }
    ...
    public async Task<PayrollProject100ProposeCorrectionResult> ProposeTimeCorrectionAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new PayrollProject100ProposeCorrectionResult(false, "Reden is verplicht.", null, null);
        }

        if (newEnd <= newStart)
        {
            return new PayrollProject100ProposeCorrectionResult(
                false,
        ...
        if (detail is null)
        {
            return new PayrollProject100ProposeCorrectionResult(false, "Admin-case niet gevonden.", null, null);
        }
        ...
        if (target is null)
        {
            var unsupported = detail.CorrectionTargets.FirstOrDefault(item =>
                item.PerformanceId == performanceId
                && item.CorrectionCapability == PayrollProject100CorrectionCapability.UnsupportedActivity);
            return new PayrollProject100ProposeCorrectionResult(
                false,
        ...
        {
            return new PayrollProject100ProposeCorrectionResult(false, "Activity type ontbreekt.", null, null);
        }

        if (target.CurrentStart is null || target.CurrentEnd is null)
        {
            return new PayrollProject100ProposeCorrectionResult(false, "Huidige VAN/TOT ontbreekt.", null, null);
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"Project100Workbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"p100-adjust:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Project 100 tijdscorrectie (voorstel)",
            reason.Trim(),
            [performanceId],
            proposedStart,
            proposedEnd,
        ...
        existing.SourceRevision =
            $"p100:{performanceId}:{target.CurrentStart:O}:{target.CurrentEnd:O}:{target.ActivityType}:{target.HfdTaakId}";
        ...
        logger.LogInformation(
            "Created Project100 adjust proposal {ActionId} perf={PerformanceId} activity={Activity} (ExecutionEnabled gate applies).",
            existing.ActionId,
            performanceId,
            target.ActivityType);
    ...
    public async Task<PayrollProject100ProposeCorrectionResult> ProposeDeletePerformanceAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (booked is null)
        {
            return new PayrollProject100ProposeCorrectionResult(false, "Geboekte prestatie niet gevonden.", null, null);
        }
        ...
        {
            return new PayrollProject100ProposeCorrectionResult(false, "ProjectId ontbreekt.", null, "IncompleteTarget");
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"Project100Workbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"p100-delete:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Project 100 prestatie verwijderen (voorstel)",
            reason.Trim(),
            [performanceId],
        ...
        existing.SourceRevision =
            $"p100-delete:{performanceId}:{deleteTarget.CurrentStart:O}:{deleteTarget.CurrentEnd:O}:{projectId}:{booked.HfdTaakId}";
        existing.Comment = reason.Trim();
        existing.UpdatedAtUtc = now;
        ...
        queueCache.InvalidateMonth(year, month);
        logger.LogInformation(
            "Created Project100 delete proposal {ActionId} perf={PerformanceId} (DeletePerformanceEnabled gate applies).",
            existing.ActionId,
            performanceId);
    ...
    private async Task<(PayrollProject100CaseDetail Detail, PayrollProject100WorkbenchMetrics Metrics)> LoadCoreDetailAsync(
        PayrollAdminCase adminCase,
        PayrollProject100MonthContext monthContext,
        CancellationToken cancellationToken)
    {
        if (!monthContext.TryGetDay(adminCase.ResourceId, adminCase.Date, out var day))
        {
            day = new Project100DaySources([], []);
        }
        ...
        var peerSessionPerformances = CollectPeerSessionPerformances(
            monthContext,
        ...
        var activities = new Dictionary<long, PayrollProject100ResolvedActivity>();
        var probe = PayrollProject100WorkbenchBuilder.BuildDetail(
            adminCase,
            performances,
            planning,
            null,
            ...
            peerSessionPerformances: peerSessionPerformances);
        ...
        foreach (var row in probe.BookedRows)
        {
            ...
            activities[perf.SourceEntryId] = new PayrollProject100ResolvedActivity(
                perf.SourceEntryId,
        ...
        var metrics = new PayrollProject100WorkbenchMetrics(
            0,
            0,
            PowerFleetApiCalls: 0,
    ...
    private static NormalizedPerformanceEntry[] CollectPeerSessionPerformances(
        PayrollProject100MonthContext monthContext,
        string resourceId,
        DateOnly date,
        IReadOnlyList<NormalizedPerformanceEntry> ownPerformances,
        IReadOnlyList<PayrollPlanningReservation> ownPlanning)
    {
        var supportingCalendarIds = ownPerformances
            .Where(item => item.ProjectNumber == 100)
            .SelectMany(perf => SpecialProjectTimeControl.FindMatchingReservations(perf, ownPlanning))
            .Select(item => item.IdCalendar)
            .ToHashSet();

        var peers = new List<NormalizedPerformanceEntry>();
        foreach (var (key, day) in monthContext.DaysByResourceDate)
        {
            if (!key.EndsWith($"|{date:yyyyMMdd}", StringComparison.Ordinal)
                || key.StartsWith($"{resourceId.Trim()}|", StringComparison.Ordinal))
            {
                continue;
            }

            foreach (var perf in day.Performances.Where(item => item.ProjectNumber == 100 && !item.IsAbsence))
            {
                if (supportingCalendarIds.Count > 0)
                {
                    var peerMatches = SpecialProjectTimeControl.FindMatchingReservations(perf, day.Planning);
                    if (peerMatches.Any(item => supportingCalendarIds.Contains(item.IdCalendar)))
                    {
                        peers.Add(perf);
                        continue;
                    }
                ...
                // Fallback: same day Project100 with overlapping clock when calendars unknown.
                if (supportingCalendarIds.Count == 0
                    && ownPerformances.Any(own =>
                        own.ProjectNumber == 100
                        && own.Start is not null
                        && own.End is not null
                        && perf.Start is not null
                        && perf.End is not null
                        && own.Start < perf.End
                        && own.End > perf.Start))
                {
                    peers.Add(perf);
    ...
    private async Task<PayrollProject100MonthContext> EnsureMonthContextAsync(
        int year,
        int month,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                planning = await planningSource.ReadSharedAttendeeReservationsAsync(
                    periodStart,
                    periodEnd,
                    resourceIds,
                    cancellationToken);
                ...
                var expandedResourceIds = planning
                    .Select(item => item.ResourceId)
                    .Where(item => !string.IsNullOrWhiteSpace(item))
                    .Concat(resourceIds)
                    .Distinct(StringComparer.Ordinal)
            ...
            var days = new Dictionary<string, Project100DaySources>(StringComparer.Ordinal);
            foreach (var group in performances.GroupBy(item => PayrollProject100MonthContext.DayKey(item.ResourceId, item.Date)))
            {
                days[group.Key] = new Project100DaySources(group.ToArray(), []);
            }

            foreach (var group in planning.GroupBy(item => PayrollProject100MonthContext.DayKey(item.ResourceId, item.Date)))
            {
            ...
            {
                var key = PayrollProject100MonthContext.DayKey(item.ResourceId, item.Date);
                if (!days.TryGetValue(key, out var daySources))
                {
                    daySources = new Project100DaySources([], []);
                    days[key] = daySources;
                }
            ...
            sw.Stop();
            var context = new PayrollProject100MonthContext(
                year,
                month,
                revision,
                periodStart,
                ...
                days,
                bonMemos,
                new PayrollProject100MonthContextMetrics(
                    perfQueries,
                    planQueries,
            ...
            logger.LogInformation(
                "Project100 month context built {Year}-{Month:00} resources={Resources} days={Days} ms={Ms} perfQ={PerfQ} planQ={PlanQ} bonQ={BonQ}",
                year,
                month,
                resourceIds.Length,
                days.Count,
        ...
        CancellationToken cancellationToken)
    {
        if (!warmBonFromMonth)
        {
            ...
            foreach (var item in cases)
            {
                ...
                var preview = PayrollProject100WorkbenchBuilder.BuildQueueTechnicianPreview(prestDesc, prestMemo, null);
        ...
        foreach (var item in cases)
        {
            ...
            var prestDesc = item.PerformanceDescriptionSummary
                ?? item.Performances.Select(p => p.PerformanceDescription).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var prestMemo = item.Performances.Select(p => p.PerformanceMemo).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var preview = PayrollProject100WorkbenchBuilder.BuildQueueTechnicianPreview(
                prestDesc,
                prestMemo,
                bonMemo);
            if (preview is not null)
