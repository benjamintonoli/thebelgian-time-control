...
internal sealed class PayrollProject200WorkbenchService(
    ...
    PayrollProject200GpsCache gpsCache,
    PayrollProject200GpsContextCache gpsContextCache,
    PayrollProject200DaySourceCache daySourceCache,
    PayrollProject200QueueCache queueCache,
    PayrollProject200BonMemoCache bonMemoCache,
    PayrollProject200MonthContextCache monthContextCache,
    PayrollProject200HfdCache hfdCache,
    ...
    ILogger<PayrollProject200WorkbenchService> logger) : IPayrollProject200WorkbenchService
{
    ...
    public async Task<PayrollProject200WorkbenchPage> GetShellAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
    ...
    {
        EnsureEnabled();
        var scopedFilter = filter with { Category = PayrollReviewCategory.Project200 };
        ...
        return new PayrollProject200WorkbenchPage(
            year,
            month,
            queue.AdminSummary,
            cases,
            ...
            Detail: null,
            new PayrollProject200WorkbenchMetrics(0, 0, 0, GpsDeferred: true, MonthContextHit: ready),
            previews,
            MonthContextPending: !ready,
            MonthContextReady: ready);
    }

    public async Task<PayrollProject200WorkbenchPage> GetCoreDetailAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
        CancellationToken cancellationToken)
    {
        ...
        if (selected is null)
        {
            return new PayrollProject200WorkbenchPage(
                ...
                null,
                null,
                new PayrollProject200WorkbenchMetrics(
    ...
    public async Task<PayrollProject200GpsLoadResult> GetGpsContextAsync(
        int year,
        int month,
        string adminCaseKey,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (gpsContextCache.TryGet(adminCaseKey, out var cachedContext) && cachedContext is not null)
        {
            ...
            if (prefetchNext)
            {
                var queue = await GetAdminQueueCachedAsync(
                    year,
                    month,
                    new PayrollReviewQueueFilter(PayrollReviewCategory.Project200, Scope: PayrollReviewQueueScope.All),
                    cancellationToken);
            ...
            return new PayrollProject200GpsLoadResult(
                adminCaseKey,
                string.Empty,
                default,
                cachedContext,
        ...
        if (selected is null)
        {
            return new PayrollProject200GpsLoadResult(
                ...
                new PayrollProject200GpsContext(
                    false,
                    PayrollProject200WorkbenchBuilder.MissingGpsSummary,
                    [],
                    "Missing",
                    null,
                    []),
        ...
        var gpsContext = PayrollProject200WorkbenchBuilder.BuildDetail(
            selected,
            performances,
            [],
            evidence,
    ...
    public PayrollProject200GpsCacheHint GetGpsCacheHint(string resourceId, DateOnly workDate)
    {
        ...
        {
            return PayrollProject200GpsCacheHint.Unknown;
        }

        if (evidence is null || !evidence.HasUsableTrips)
        {
            return PayrollProject200GpsCacheHint.Unavailable;
        }

        return PayrollProject200GpsCacheHint.Available;
    }
    ...
    public async Task<PayrollProject200ProposeCorrectionResult> ProposeTimeCorrectionAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new PayrollProject200ProposeCorrectionResult(false, "Reden is verplicht.", null, null);
        }

        if (newEnd <= newStart)
        {
            return new PayrollProject200ProposeCorrectionResult(
                false,
        ...
        if (detail is null)
        {
            return new PayrollProject200ProposeCorrectionResult(false, "Admin-case niet gevonden.", null, null);
        }
        ...
        if (target is null)
        {
            var unsupported = detail.CorrectionTargets.FirstOrDefault(item =>
                item.PerformanceId == performanceId
                && item.CorrectionCapability == PayrollProject200CorrectionCapability.UnsupportedActivity);
            return new PayrollProject200ProposeCorrectionResult(
                false,
        ...
        {
            return new PayrollProject200ProposeCorrectionResult(false, "Activity type ontbreekt.", null, null);
        }

        if (target.CurrentStart is null || target.CurrentEnd is null)
        {
            return new PayrollProject200ProposeCorrectionResult(false, "Huidige VAN/TOT ontbreekt.", null, null);
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"Project200Workbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"p200-adjust:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Project 200 tijdscorrectie (voorstel)",
            reason.Trim(),
            [performanceId],
            proposedStart,
            proposedEnd,
        ...
        existing.SourceRevision =
            $"p200:{performanceId}:{target.CurrentStart:O}:{target.CurrentEnd:O}:{target.ActivityType}:{target.HfdTaakId}";
        ...
        logger.LogInformation(
            "Created Project200 adjust proposal {ActionId} perf={PerformanceId} activity={Activity} (ExecutionEnabled gate applies).",
            existing.ActionId,
            performanceId,
            target.ActivityType);
    ...
    public async Task<PayrollProject200ProposeCorrectionResult> ProposeDeletePerformanceAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var deleteTarget = detail.CorrectionTargets.FirstOrDefault(item =>
            item.PerformanceId == performanceId
            && item.CorrectionCapability == PayrollProject200CorrectionCapability.SupportedDelete);
        ...
        if (booked is null)
        {
            return new PayrollProject200ProposeCorrectionResult(false, "Geboekte prestatie niet gevonden.", null, null);
        }
        ...
        {
            return new PayrollProject200ProposeCorrectionResult(false, "ProjectId ontbreekt.", null, "IncompleteTarget");
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"Project200Workbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"p200-delete:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Project 200 prestatie verwijderen (voorstel)",
            reason.Trim(),
            [performanceId],
        ...
        existing.SourceRevision =
            $"p200-delete:{performanceId}:{deleteTarget.CurrentStart:O}:{deleteTarget.CurrentEnd:O}:{projectId}:{booked.HfdTaakId}";
        existing.Comment = reason.Trim();
        existing.UpdatedAtUtc = now;
        ...
        queueCache.InvalidateMonth(year, month);
        logger.LogInformation(
            "Created Project200 delete proposal {ActionId} perf={PerformanceId} (DeletePerformanceEnabled gate applies).",
            existing.ActionId,
            performanceId);
    ...
    private async Task<(PayrollProject200CaseDetail Detail, PayrollProject200WorkbenchMetrics Metrics)> LoadCoreDetailAsync(
        PayrollAdminCase adminCase,
        PayrollProject200MonthContext monthContext,
        CancellationToken cancellationToken)
    {
        if (!monthContext.TryGetDay(adminCase.ResourceId, adminCase.Date, out var day))
        {
            day = new Project200DaySources([], []);
        }
        ...
        var probe = PayrollProject200WorkbenchBuilder.BuildDetail(
            adminCase,
            performances,
            planning,
            null,
        ...
        foreach (var row in probe.BookedRows)
        {
            ...
            var resolved = PayrollProject200ActivityResolver.Resolve(perf, def);
            activities[perf.SourceEntryId] = new PayrollProject200ResolvedActivity(
                perf.SourceEntryId,
        ...
        var metrics = new PayrollProject200WorkbenchMetrics(
            0,
            0,
            PowerFleetApiCalls: 0,
    ...
    private async Task<PayrollProject200MonthContext> EnsureMonthContextAsync(
        int year,
        int month,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var days = new Dictionary<string, Project200DaySources>(StringComparer.Ordinal);
            foreach (var group in performances.GroupBy(item => PayrollProject200MonthContext.DayKey(item.ResourceId, item.Date)))
            {
                days[group.Key] = new Project200DaySources(group.ToArray(), []);
            }

            foreach (var group in planning.GroupBy(item => PayrollProject200MonthContext.DayKey(item.ResourceId, item.Date)))
            {
            ...
            {
                var key = PayrollProject200MonthContext.DayKey(item.ResourceId, item.Date);
                if (!days.TryGetValue(key, out var daySources))
                {
                    daySources = new Project200DaySources([], []);
                    days[key] = daySources;
                }
            ...
            sw.Stop();
            var context = new PayrollProject200MonthContext(
                year,
                month,
                revision,
                periodStart,
                ...
                days,
                bonMemos,
                new PayrollProject200MonthContextMetrics(
                    perfQueries,
                    planQueries,
            ...
            logger.LogInformation(
                "Project200 month context built {Year}-{Month:00} resources={Resources} days={Days} ms={Ms} perfQ={PerfQ} planQ={PlanQ} bonQ={BonQ}",
                year,
                month,
                resourceIds.Length,
                days.Count,
        ...
        CancellationToken cancellationToken)
    {
        if (!warmBonFromMonth)
        {
            ...
            foreach (var item in cases)
            {
                ...
                var preview = PayrollProject200WorkbenchBuilder.BuildQueueTechnicianPreview(prestDesc, prestMemo, null);
        ...
        foreach (var item in cases)
        {
            ...
            var prestDesc = item.PerformanceDescriptionSummary
                ?? item.Performances.Select(p => p.PerformanceDescription).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var prestMemo = item.Performances.Select(p => p.PerformanceMemo).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var preview = PayrollProject200WorkbenchBuilder.BuildQueueTechnicianPreview(
                prestDesc,
                prestMemo,
                bonMemo);
            if (preview is not null)
