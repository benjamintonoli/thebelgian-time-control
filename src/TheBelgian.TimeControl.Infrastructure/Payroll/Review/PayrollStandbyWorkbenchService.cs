...
internal sealed class PayrollStandbyWorkbenchService(
    ...
    PayrollStandbyGpsDayCache gpsCache,
    PayrollStandbyGpsContextCache gpsContextCache,
    PayrollStandbyDaySourceCache daySourceCache,
    PayrollStandbyQueueCache queueCache,
    PayrollStandbyBonMemoCache bonMemoCache,
    PayrollStandbyMonthContextCache monthContextCache,
    PayrollStandbyHfdCache hfdCache,
    ...
    ILogger<PayrollStandbyWorkbenchService> logger) : IPayrollStandbyWorkbenchService
{
    ...
    public async Task<PayrollStandbyWorkbenchPage> GetShellAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
    ...
    {
        EnsureEnabled();
        var scopedFilter = filter with { Category = PayrollReviewCategory.Standby };
        ...
        return new PayrollStandbyWorkbenchPage(
            year,
            month,
            queue.AdminSummary,
            cases,
            ...
            Detail: null,
            new PayrollStandbyWorkbenchMetrics(0, 0, 0, GpsDeferred: true, MonthContextHit: ready),
            previews,
            MonthContextPending: !ready,
            MonthContextReady: ready);
    }

    public async Task<PayrollStandbyWorkbenchPage> GetCoreDetailAsync(
        int year,
        int month,
        PayrollReviewQueueFilter filter,
        string? selectedAdminCaseKey,
        CancellationToken cancellationToken)
    {
        ...
        if (selected is null)
        {
            return new PayrollStandbyWorkbenchPage(
                ...
                null,
                null,
                new PayrollStandbyWorkbenchMetrics(
    ...
    public async Task<PayrollStandbyGpsLoadResult> GetGpsContextAsync(
        int year,
        int month,
        string adminCaseKey,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (gpsContextCache.TryGet(adminCaseKey, out var cachedContext) && cachedContext is not null)
        {
            ...
            if (prefetchNext)
            {
                var queue = await GetAdminQueueCachedAsync(
                    year,
                    month,
                    new PayrollReviewQueueFilter(PayrollReviewCategory.Standby, Scope: PayrollReviewQueueScope.All),
                    cancellationToken);
            ...
            return new PayrollStandbyGpsLoadResult(
                adminCaseKey,
                string.Empty,
                default,
                cachedContext,
        ...
        if (selected is null)
        {
            return new PayrollStandbyGpsLoadResult(
                ...
                new PayrollStandbyGpsContext(
                    false,
                    PayrollStandbyWorkbenchBuilder.MissingGpsSummary,
                    [],
                    "Missing",
                    null,
                    []),
        ...
        var gpsContext = PayrollStandbyWorkbenchBuilder.BuildDetail(
            selected,
            performances,
            [],
            evidence,
    ...
    public PayrollStandbyGpsCacheHint GetGpsCacheHint(string resourceId, DateOnly workDate)
    {
        ...
        {
            return PayrollStandbyGpsCacheHint.Unknown;
        }

        if (evidence is null || !evidence.HasUsableTrips)
        {
            return PayrollStandbyGpsCacheHint.Unavailable;
        }

        return PayrollStandbyGpsCacheHint.Available;
    }
    ...
    public async Task<PayrollStandbyProposeCorrectionResult> ProposeTimeCorrectionAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new PayrollStandbyProposeCorrectionResult(false, "Reden is verplicht.", null, null);
        }

        if (newEnd <= newStart)
        {
            return new PayrollStandbyProposeCorrectionResult(
                false,
        ...
        if (detail is null)
        {
            return new PayrollStandbyProposeCorrectionResult(false, "Admin-case niet gevonden.", null, null);
        }
        ...
        if (target is null)
        {
            var unsupported = detail.CorrectionTargets.FirstOrDefault(item =>
                item.PerformanceId == performanceId
                && item.CorrectionCapability == PayrollStandbyCorrectionCapability.UnsupportedActivity);
            return new PayrollStandbyProposeCorrectionResult(
                false,
        ...
        {
            return new PayrollStandbyProposeCorrectionResult(false, "Activity type ontbreekt.", null, null);
        }

        if (target.CurrentStart is null || target.CurrentEnd is null)
        {
            return new PayrollStandbyProposeCorrectionResult(false, "Huidige VAN/TOT ontbreekt.", null, null);
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"StandbyWorkbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"standby-adjust:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Wachtdienst tijdscorrectie (voorstel)",
            reason.Trim(),
            [performanceId],
            proposedStart,
            proposedEnd,
        ...
        existing.SourceRevision =
            $"standby:{performanceId}:{target.CurrentStart:O}:{target.CurrentEnd:O}:{target.ActivityType}:{target.HfdTaakId}";
        ...
        logger.LogInformation(
            "Created Standby adjust proposal {ActionId} perf={PerformanceId} activity={Activity} (ExecutionEnabled gate applies).",
            existing.ActionId,
            performanceId,
            target.ActivityType);
    ...
    public async Task<PayrollStandbyProposeCorrectionResult> ProposeDeletePerformanceAsync(
        int year,
        int month,
        string adminCaseKey,
        long performanceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var deleteTarget = detail.CorrectionTargets.FirstOrDefault(item =>
            item.PerformanceId == performanceId
            && item.CorrectionCapability == PayrollStandbyCorrectionCapability.SupportedDelete);
        ...
        if (booked is null)
        {
            return new PayrollStandbyProposeCorrectionResult(false, "Geboekte prestatie niet gevonden.", null, null);
        }
        ...
        {
            return new PayrollStandbyProposeCorrectionResult(false, "ProjectId ontbreekt.", null, "IncompleteTarget");
        }
        ...
        var findingKey = detail.AdminCase.FindingKeys.Count > 0
            ...
            : $"StandbyWorkbench:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        var findingId = detail.AdminCase.FindingIds.Count > 0 ? detail.AdminCase.FindingIds[0] : 0;
        var actionKey = $"standby-delete:{detail.AdminCase.ResourceId}:{date:yyyyMMdd}:{performanceId}";
        ...
        var evidence = new PayrollActionEvidenceSnapshot(
            ...
            "Wachtdienst prestatie verwijderen (voorstel)",
            reason.Trim(),
            [performanceId],
        ...
        existing.SourceRevision =
            $"standby-delete:{performanceId}:{deleteTarget.CurrentStart:O}:{deleteTarget.CurrentEnd:O}:{projectId}:{booked.HfdTaakId}";
        existing.Comment = reason.Trim();
        existing.UpdatedAtUtc = now;
        ...
        queueCache.InvalidateMonth(year, month);
        logger.LogInformation(
            "Created Standby delete proposal {ActionId} perf={PerformanceId} (DeletePerformanceEnabled gate applies).",
            existing.ActionId,
            performanceId);
    ...
    private async Task<(PayrollStandbyCaseDetail Detail, PayrollStandbyWorkbenchMetrics Metrics)> LoadCoreDetailAsync(
        PayrollAdminCase adminCase,
        PayrollStandbyMonthContext monthContext,
        CancellationToken cancellationToken)
    {
        if (!monthContext.TryGetDay(adminCase.ResourceId, adminCase.Date, out var day))
        {
            day = new StandbyDaySources([], []);
        }
        ...
        var activities = new Dictionary<long, PayrollStandbyResolvedActivity>();
        var probe = PayrollStandbyWorkbenchBuilder.BuildDetail(
            adminCase,
            performances,
            planning,
            null,
        ...
        foreach (var row in probe.BookedRows)
        {
            ...
            var resolved = PayrollStandbyWorkbenchActivityResolver.Resolve(perf, def);
            activities[perf.SourceEntryId] = new PayrollStandbyResolvedActivity(
                perf.SourceEntryId,
        ...
        var detail = PayrollStandbyWorkbenchBuilder.BuildDetail(
            adminCase,
            performances,
            planning,
        ...
        var metrics = new PayrollStandbyWorkbenchMetrics(
            0,
            0,
            PowerFleetApiCalls: 0,
    ...
    private async Task<PayrollStandbyMonthContext> EnsureMonthContextAsync(
        int year,
        int month,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var days = new Dictionary<string, StandbyDaySources>(StringComparer.Ordinal);
            foreach (var group in performances.GroupBy(item => PayrollStandbyMonthContext.DayKey(item.ResourceId, item.Date)))
            {
                days[group.Key] = new StandbyDaySources(group.ToArray(), []);
            }

            foreach (var group in planning.GroupBy(item => PayrollStandbyMonthContext.DayKey(item.ResourceId, item.Date)))
            {
            ...
            {
                var key = PayrollStandbyMonthContext.DayKey(item.ResourceId, item.Date);
                if (!days.TryGetValue(key, out var daySources))
                {
                    daySources = new StandbyDaySources([], []);
                    days[key] = daySources;
                }
            ...
            sw.Stop();
            var context = new PayrollStandbyMonthContext(
                year,
                month,
                revision,
                periodStart,
                ...
                days,
                bonMemos,
                new PayrollStandbyMonthContextMetrics(
                    perfQueries,
                    planQueries,
            ...
            logger.LogInformation(
                "Standby month context built {Year}-{Month:00} resources={Resources} days={Days} ms={Ms} perfQ={PerfQ} planQ={PlanQ} bonQ={BonQ}",
                year,
                month,
                resourceIds.Length,
                days.Count,
        ...
        CancellationToken cancellationToken)
    {
        if (!warmBonFromMonth)
        {
            ...
            foreach (var item in cases)
            {
                ...
                var preview = PayrollStandbyWorkbenchBuilder.BuildQueueTechnicianPreview(prestDesc, prestMemo, null);
        ...
        foreach (var item in cases)
        {
            ...
            var prestDesc = item.PerformanceDescriptionSummary
                ?? item.Performances.Select(p => p.PerformanceDescription).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var prestMemo = item.Performances.Select(p => p.PerformanceMemo).FirstOrDefault(t => !string.IsNullOrWhiteSpace(t));
            var preview = PayrollStandbyWorkbenchBuilder.BuildQueueTechnicianPreview(
                prestDesc,
                prestMemo,
                bonMemo);
            if (preview is not null)
