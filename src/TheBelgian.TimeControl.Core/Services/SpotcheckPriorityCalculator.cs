...
public static class SpotcheckPriorityCalculator
{
    public static SpotcheckPriorityTier FromDeviationMinutes(int maxAbsDeviationMinutes)
    {
        ...
        if (abs >= 30)
        {
            return SpotcheckPriorityTier.High;
        }
    ...
    /// by at most 3 minutes (informative band). Used only for recurring pattern flags.
    /// </summary>
    ...
    public static IReadOnlyList<AdminReviewCase> ApplyFilterAndSort(
        IReadOnlyList<AdminReviewCase> cases,
        AdminReviewFilter filter)
    {
        IEnumerable<AdminReviewCase> query = cases;
        ...
        {
            query = query.Where(item => item.Priority == SpotcheckPriorityTier.High);
        }
