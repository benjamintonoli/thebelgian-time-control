...
/// <summary>
/// Pure Wachtdienst workbench detail assembly. Evidence-only; never auto-validates.
/// </summary>
public static class PayrollStandbyWorkbenchBuilder
{
    ...
    public static PayrollStandbyCaseDetail BuildDetail(
        ...
        IReadOnlyDictionary<long, PayrollStandbyResolvedActivity>? activityByPerformanceId = null,
        ...
        bool includeTechnicalDiagnostics = false)
    {
        ...
        var bookedRows = selected
            ...
            .Select(item => new PayrollStandbyBookedRow(
        ...
        var planningRows = dayPlanning
            ...
            .Select(item => new PayrollStandbyPlanningRow(
        ...
        var hasSupported = corrections.Any(item =>
            item.CorrectionCapability == PayrollStandbyCorrectionCapability.SupportedVanTot);

        return new PayrollStandbyCaseDetail(
            AdminCase: adminCase,
    ...
    public static PayrollStandbyPlanningComparison BuildPlanningComparison(
        ...
        IReadOnlyList<PayrollPlanningReservation> supporting)
    {
        ...
        {
            return new PayrollStandbyPlanningComparison(
                HasMatchingPlanning: false,
                MatchingCount: 0,
                BookedSummary: bookedSummary,
                PlannedSummary: null,
                DifferenceSummary: "Geen passende planning gevonden",
                TopSummary: $"Wachtdienst geboekt {bookedSummary}. Geen passende planning gevonden.",
        ...
        var top = supporting.Count == 1
            ? $"Wachtdienst geboekt {bookedSummary}. Planning {plannedSummary}. Verschil {diffSummary}."
            : $"Wachtdienst geboekt {bookedSummary}. {supporting.Count} passende reservaties (max {FormatHoursShort(plannedHours)}). Verschil {diffSummary}.";
    ...
    public static IReadOnlyList<PayrollStandbyDayTimelineEntry> BuildUnifiedDayTimeline(
        ...
        KnownLocationCatalog? knownLocations = null)
    {
        ...
        dayPlanning ??= [];
        knownLocations ??= KnownLocationCatalog.Empty;
        var entries = new List<PayrollStandbyDayTimelineEntry>();
        ...
        foreach (var plan in dayPlanning.Where(item => item.Classification != PayrollPlanningClassification.Absence))
        {
            ...
            entries.Add(new PayrollStandbyDayTimelineEntry(
                SortAt: start,
                Start: start,
                End: end,
                Kind: PayrollStandbyDayTimelineKind.Planning,
                Badge: "PLANNING",
                Title: BuildPlanningLabel(plan),
                Subtitle: null,
                IsSelectedStandby: false));
        }
        ...
        foreach (var perf in dayPerformances.Where(item => !item.IsCalendarSynthetic && !item.IsAbsence && item.Start is not null))
        {
            ...
            entries.Add(new PayrollStandbyDayTimelineEntry(
                ...
                Start: perf.Start,
                End: perf.End,
                Kind: is300 ? PayrollStandbyDayTimelineKind.Standby : PayrollStandbyDayTimelineKind.Performance,
                Badge: is300 ? "Standby" : "PRESTATIE",
                Title: title,
                Subtitle: subtitle,
                IsSelectedStandby: is300));
        }
    ...
    /// <summary>
    /// Concise default review window around Wachtdienst. Full day remains in FullDay.
    /// </summary>
    public static PayrollStandbyFocusedContext BuildFocusedReviewContext(
        ...
        IReadOnlyList<NormalizedPerformanceEntry> selectedPerformances)
    {
        ...
        {
            var bookingOnly = ordered.Where(item => item.IsSelectedStandby).ToArray();
            return new PayrollStandbyFocusedContext(
                ContextSummary: null,
                Before: [],
                Booking: bookingOnly,
                Intervention: [],
        ...
        var booking = ordered.Where(item => item.IsSelectedStandby).ToArray();
        var before = BuildFocusedBefore(ordered, windowStart);
        var after = BuildFocusedAfter(ordered, windowStart, windowEnd, before);
        var intervention = ordered
            .Where(item =>
                (item.Kind is PayrollStandbyDayTimelineKind.Gps or PayrollStandbyDayTimelineKind.Intervention)
        ...
        DateTimeOffset windowEnd)
    {
        ...
        parts.Add("Wachtdienst geboekt " + FormatClock(windowStart) + "–" + FormatClock(windowEnd));
        ...
        var nextWork = after.FirstOrDefault(item =>
            item.Kind is PayrollStandbyDayTimelineKind.Planning or PayrollStandbyDayTimelineKind.Performance);
        if (nextWork is not null)
        {
            var kind = nextWork.Kind == PayrollStandbyDayTimelineKind.Planning
                ? "Volgende planning"
                : "Volgende prestatie";
    ...
    private static PayrollStandbyDayTimelineEntry[] BuildFocusedBefore(
        IReadOnlyList<PayrollStandbyDayTimelineEntry> ordered,
        DateTimeOffset windowStart)
    {
        var gps = ordered.Where(item => item.Kind == PayrollStandbyDayTimelineKind.Gps).ToArray();
        if (gps.Length == 0)
        {
            return [];
        }
    ...
    private static PayrollStandbyDayTimelineEntry[] BuildFocusedAfter(
        IReadOnlyList<PayrollStandbyDayTimelineEntry> ordered,
        DateTimeOffset windowStart,
        DateTimeOffset windowEnd,
        IReadOnlyList<PayrollStandbyDayTimelineEntry> before)
    {
        var after = new List<PayrollStandbyDayTimelineEntry>();
        var gps = ordered
            .Where(item => item.Kind == PayrollStandbyDayTimelineKind.Gps && !before.Contains(item))
            .OrderBy(item => item.SortAt)
            .ToArray();
        ...
        var nextPlanning = ordered
            .Where(item => item.Kind == PayrollStandbyDayTimelineKind.Planning)
        ...
        var nextPerformance = ordered
            .Where(item => item.Kind == PayrollStandbyDayTimelineKind.Performance && !item.IsSelectedStandby)
            .Where(item => item.SortAt >= windowEnd)
        ...
        PayrollStandbyDayTimelineEntry? nextWork = nextPlanning ?? nextPerformance;
        if (nextWork is not null && !after.Contains(nextWork))
        {
            after.Add(nextWork with
            {
                Subtitle = nextWork.Kind == PayrollStandbyDayTimelineKind.Planning
                    ? "Volgende planning"
                    : "Volgende prestatie",
            });
        }
    ...
    private static bool EventOverlapsWindow(
        PayrollStandbyDayTimelineEntry entry,
        DateTimeOffset windowStart,
        DateTimeOffset windowEnd)
    {
    ...
    private static bool IsGpsArrival(PayrollStandbyDayTimelineEntry entry) =>
        entry.Kind == PayrollStandbyDayTimelineKind.Gps
        && entry.Title.StartsWith("Aankomst", StringComparison.Ordinal);

    private static bool IsGpsDeparture(PayrollStandbyDayTimelineEntry entry) =>
        entry.Kind == PayrollStandbyDayTimelineKind.Gps
    ...
    /// </summary>
    public static IReadOnlyList<PayrollStandbyDayTimelineEntry> BuildGpsTripChainEntries(
        ...
        for (var i = 0; i < ordered.Length; i++)
        {
            ...
            if (i > 0)
            {
                ...
                if (gapMinutes >= 2)
                {
                    ...
                    entries.Add(new PayrollStandbyDayTimelineEntry(
                        ...
                        Start: previous.End,
                        End: trip.Start,
                        Kind: PayrollStandbyDayTimelineKind.Gps,
                        Badge: "GPS",
            ...
            else if (trip.Start >= selectedStart && trip.Start < selectedEnd)
            {
                ...
                departureSubtitle = minsBeforeEnd > 0
                    ...
                    : "Vertrek tijdens wachtdienst-boeking";
            }
            else if (trip.Start == selectedEnd)
            {
                departureSubtitle = "Vertrek tijdens wachtdienst-boeking";
            }
            else
            {
        ...
        DateTimeOffset selectedEnd)
    {
        ...
        {
            return "Vertrek tijdens wachtdienst-boeking";
        }
            ...
            && eventStart < selectedStart)
        {
            ...
            return "Aankomst " + minsBeforeEnd + " min vóór einde wachtdienst-boeking";
        }

        if (relation == "During")
        {
            return "Overlapt " + overlapMinutes + " min met wachtdienst-boeking";
        }
    ...
    public static PayrollStandbyTechnicianContext BuildTechnicianContext(
        IReadOnlyList<NormalizedPerformanceEntry> selectedPerformances,
        ...
        string? fallbackBonNr = null)
    {
        ...
        var remarks = selectedPerformances
            ...
            .Select(item =>
            {
                ...
                return new PayrollStandbyPerformanceRemark(
                    item.SourceEntryId,
                    item.Start,
                    item.End,
                    item.AtlHoursRaw,
        ...
        return new PayrollStandbyTechnicianContext(
            BonTechnicianRemark: bonText,
            BonNr: string.IsNullOrWhiteSpace(bonNr) ? null : bonNr.Trim(),
            BonRemarkSourceField: PayrollStandbyTechnicianContext.BonMemoSourceField,
    ...
    private static PayrollStandbyTimelineRow[] BuildNeighborTimeline(
        ...
        IReadOnlyList<NormalizedPerformanceEntry> dayPerformances)
    {
        ...
        {
            return [.. real.Select(item => ToTimelineRow(item, PayrollStandbyTimelineKind.Other, selectedIds))];
        }
        ...
        var rows = new List<PayrollStandbyTimelineRow>();
        if (previous is not null)
        {
            rows.Add(ToTimelineRow(previous, PayrollStandbyTimelineKind.Previous, selectedIds));
        }

        foreach (var item in selected.OrderBy(item => item.Start).ThenBy(item => item.SortKey))
        {
            rows.Add(ToTimelineRow(item, PayrollStandbyTimelineKind.Selected, selectedIds));
        }

        if (next is not null)
        {
            rows.Add(ToTimelineRow(next, PayrollStandbyTimelineKind.Next, selectedIds));
        }

        return [.. rows];
    }

    private static PayrollStandbyTimelineRow ToTimelineRow(
        ...
        new(
            ...
            IsSelectedStandby: selectedIds.Contains(entry.SourceEntryId));

    private static PayrollStandbyGpsContext BuildGpsContext(
        NormalizedPerformanceEntry[] selected,
        StandbyGpsDayEvidence? gps,
        KnownLocationCatalog? knownLocations = null)
    {
        ...
        {
            return new PayrollStandbyGpsContext(
                Available: false,
        ...
        var events = chain
            .Select(item => new PayrollStandbyGpsEvent(
        ...
        var summary =
            ...
            + PayrollStandbyCaseDetail.GpsNeverValidatesNote;
    ...
    private static List<PayrollStandbyCorrectionTarget> BuildCorrectionTargets(
        NormalizedPerformanceEntry[] selected,
        IReadOnlyDictionary<long, PayrollStandbyResolvedActivity>? activityByPerformanceId,
        TimeOnly? suggestedAdjustStart = null,
        TimeOnly? suggestedAdjustEnd = null)
    {
        var targets = new List<PayrollStandbyCorrectionTarget>();
        foreach (var item in selected.OrderBy(item => item.Start).ThenBy(item => item.SortKey))
        {
            if (activityByPerformanceId is not null
                && activityByPerformanceId.TryGetValue(item.SourceEntryId, out var resolved))
            {
                targets.Add(new PayrollStandbyCorrectionTarget(
                    PerformanceId: item.SourceEntryId,
                    ...
                    CorrectionCapability: resolved.Supported
                        ? PayrollStandbyCorrectionCapability.SupportedVanTot
                        : PayrollStandbyCorrectionCapability.UnsupportedActivity,
            ...
            else if (item.HfdTaakId == PayrollStandbyActivityTypes.WaitingMainTaskExternalId)
            {
                targets.Add(new PayrollStandbyCorrectionTarget(
                    ...
                    CorrectionCapability: PayrollStandbyCorrectionCapability.SupportedVanTot,
            ...
            else
            {
                targets.Add(new PayrollStandbyCorrectionTarget(
                    ...
                    HfdTaakId: item.HfdTaakId,
                    ActivityType: null,
                    CorrectionCapability: PayrollStandbyCorrectionCapability.UnsupportedActivity,
            ...
            targets.Add(new PayrollStandbyCorrectionTarget(
                ...
                ActivityType: item.HfdTaakId == PayrollStandbyActivityTypes.WaitingMainTaskExternalId
                    ...
                    : activityByPerformanceId is not null
                        ...
                        : null,
                CorrectionCapability: PayrollStandbyCorrectionCapability.SupportedDelete,
    ...
    private static string BuildResolvedCapabilityMessage(PayrollStandbyResolvedActivity resolved)
    {
    ...
    private static List<string> BuildTechnicalNotes(
        ...
        NormalizedPerformanceEntry[] selected,
        StandbyGpsDayEvidence? gps,
        PayrollStandbyTechnicianContext? technician = null)
    {
