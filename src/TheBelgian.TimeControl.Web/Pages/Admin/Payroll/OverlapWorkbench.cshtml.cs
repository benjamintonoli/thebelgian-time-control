...
public sealed class OverlapWorkbenchModel(
    ...
    ILogger<OverlapWorkbenchModel> logger) : PageModel
{
    private const PayrollReviewCategory Category = PayrollReviewCategory.Overlap;
    ...
    public async Task<IActionResult> OnGetDetailAsync(string? adminCaseKey, CancellationToken cancellationToken)
    {
        ...
        return Partial("_OverlapDetail", this);
    }
    ...
    public async Task<IActionResult> OnPostDecideAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new JsonResult(new
            {
                ...
                nextKey,
                openAdjust = string.Equals(
                    result.DecisionCode,
                    PayrollGuidedDecisionCodes.OverlapAdjustTime,
                    StringComparison.Ordinal)
                    || string.Equals(
                        result.DecisionCode,
                        PayrollGuidedDecisionCodes.OverlapApplyProposal,
                        StringComparison.Ordinal),
                openDelete = string.Equals(
                    result.DecisionCode,
                    PayrollGuidedDecisionCodes.OverlapDelete,
                    StringComparison.Ordinal),
            });
        }
        catch (Exception exception)
        {
            logger.LogWarning(exception, "Overlap decide failed for {AdminCaseKey}", AdminCaseKey);
    ...
    public async Task<IActionResult> OnPostProposeCorrectionAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var propose = await workbenchService.ProposeAdjustAsync(
                Year,
                Month,
                AdminCaseKey.Trim(),
                PerformanceId,
        ...
        {
            logger.LogWarning(exception, "Overlap propose correction failed.");
            return new JsonResult(new { ok = false, error = exception.Message })
            {
                StatusCode = StatusCodes.Status400BadRequest,
            };
    ...
    public async Task<IActionResult> OnPostProposeDeleteAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var propose = await workbenchService.ProposeDeleteAsync(
                Year,
                Month,
                AdminCaseKey.Trim(),
                PerformanceId,
        ...
        {
            logger.LogWarning(exception, "Overlap propose delete failed.");
            return new JsonResult(new { ok = false, error = exception.Message })
            {
                StatusCode = StatusCodes.Status400BadRequest,
            };
    ...
    public async Task<IActionResult> OnGetGpsAsync(string adminCaseKey, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new JsonResult(new
            {
                ...
                adviceNl = result.OverlapWithGps?.AdviceNl,
                kindLabel = result.OverlapWithGps is null
                    ? null
                    : PayrollIntelligenceWorkbenchBuilder.OverlapKindLabelNl(result.OverlapWithGps.Kind),
        ...
        {
            logger.LogWarning(exception, "Overlap GPS load failed for {AdminCaseKey}", adminCaseKey);
            return new JsonResult(new
            {
                ok = true,
                available = false,
